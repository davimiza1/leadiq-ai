    ...
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <nav aria-label="Main navigation">
          ...
          <button className="nav-item"><span>◎</span>Lead intelligence <em>{leads.length}</em></button>
          <button className="nav-item"><span>↗</span>Automations</button>
          <button className="nav-item"><span>◇</span>Sources</button>
      ...
      <section className="content">
        <header className="topbar">
          ...
          <div className="top-actions"><button className="icon-button" aria-label="Notifications">♢<span /></button><button className="primary-button" onClick={() => setShowForm(true)}>＋ Qualify new lead</button></div>
        </header>
        ...
        <section className="lead-panel">
          <div className="panel-header">
            ...
            <div className="panel-tools"><label className="search"><span>⌕</span><input aria-label="Search leads" placeholder="Search leads..." value={query} onChange={(e) => setQuery(e.target.value)} /></label><button className="secondary-button">⇩ Export</button></div>
          </div>
      ...
      {selected && <div className="drawer-backdrop" onMouseDown={() => setSelected(null)}><aside className="drawer" onMouseDown={(e) => e.stopPropagation()} aria-label="Lead intelligence details"><button className="modal-close" onClick={() => setSelected(null)} aria-label="Close">×</button><p className="eyebrow">AI LEAD PROFILE</p><div className="drawer-person"><span>{selected.name.split(" ").map((n) => n[0]).join("").slice(0, 2)}</span><div><h2>{selected.name}</h2><p>{selected.email}</p></div></div><div className="drawer-score"><div><small>QUALIFICATION SCORE</small><strong>{selected.score}<em>/100</em></strong></div><span className={`temp ${selected.temperature.toLowerCase()}`}>{selected.temperature} lead</span></div><div className="reason-box"><span>✦</span><div><strong>AI recommendation</strong><p>{selected.score >= 80 ? "Contact this lead within 15 minutes and offer a viewing slot. Their budget and timeline show strong purchase readiness." : "Add this prospect to a tailored nurture sequence and follow up with matching property options."}</p></div></div><dl><div><dt>Budget</dt><dd>{money.format(selected.budget)}</dd></div><div><dt>Timeline</dt><dd>{selected.timeline}</dd></div><div><dt>Property</dt><dd>{selected.property}</dd></div><div><dt>Location</dt><dd>{selected.location}</dd></div><div><dt>Source</dt><dd>{selected.source}</dd></div><div><dt>CRM status</dt><dd>{selected.status}</dd></div></dl><button className="primary-button drawer-cta">Create personalized follow-up →</button></aside></div>}
    </main>
  );
}
