...
export async function POST(request: Request) {
  ...
  if (!supabaseUrl || !supabaseAnonKey || !resendApiKey) return NextResponse.json({ error: "Email sending is not configured yet." }, { status: 503 });
  ...
  const { data: emailRecord, error: historyError } = await supabase.from("lead_emails").insert({
    user_id: userResult.user.id, lead_id: lead.id, resend_id: data?.id ?? null,
    recipient: lead.email, subject, body, status: "sent",
  ...
  await supabase.from("lead_activities").insert({ user_id: userResult.user.id, lead_id: lead.id, kind: "email_sent", description: `Email sent: ${subject}` });
  return NextResponse.json({ email: emailRecord });
}
